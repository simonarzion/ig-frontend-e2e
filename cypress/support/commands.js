const inactiveAccount = {
  ...
  password: "asdasdasd@asdasd.com",
};

const existingAccount = {
  email: "simon.villaverde@arzion.com",
  password: "simon.villaverde@arzion.com",
};
...
Cypress.Commands.add("registerExistingAccount", () => {
  cy.fixture("user").then((user) => {
    ...
    cy.get("#email")
      ...
      .should("have.value", existingAccount.email);
    cy.get("#password").type(user.password).should("have.value", user.password);
    cy.get("#password2")
      .type(user.password)
      .should("have.value", user.password);
...
Cypress.Commands.add("registerInactiveAccount", () => {
  cy.fixture("user").then((user) => {
    cy.get("#name").type(user.username).should("have.value", user.username);
    cy.get("#email")
      .type(inactiveAccount.email)
      .should("have.value", inactiveAccount.email);
    cy.get("#password").type(user.password).should("have.value", user.password);
    cy.get("#password2")
      .type(user.password)
      .should("have.value", user.password);
